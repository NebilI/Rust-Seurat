...
stopifnot(all.equal(
  ...
  SeuratRust::row_sum_dgcmatrix(x, i, nrow(m), ncol(m))
))
cat("Row stats OK\n")
...
rust <- SeuratRust::LogNorm(mat, 1e4, display_progress = FALSE)
...
cat("==> Timing: Seurat vs SeuratRust (ratio > 1 => Rust faster)...\n")
